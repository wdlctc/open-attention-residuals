...
\begin{enumerate}
    \item \textbf{What to route}: We identify the source redundancy problem in Attention Residuals---cumulative hidden states have 0.95 cosine similarity, collapsing softmax routing---and show that per-sublayer delta sources resolve it, achieving 7.5--19.2\% PPL reduction across scales (\S\ref{sec:analysis}, \S\ref{sec:experiments}).
    \item \textbf{Where to route}: We introduce per-stream V-routing, which decouples depth routing for the value stream from Q/K. A systematic $\{$cumulative, delta$\}$ $\times$ $\{$shared, per-stream$\}$ ablation shows the two dimensions are orthogonal and composable, with their combination achieving the best results at 0.011\% additional parameter cost (\S\ref{sec:per_stream}, \S\ref{sec:ablation}).
    \item \textbf{Practical deployment via fine-tuning}: Delta AttnRes's additive formulation, combined with null source initialization, enables effective fine-tuning of pretrained models with zero initialization disruption, achieving 29.2\% PPL reduction on Qwen3-0.6B (\S\ref{sec:finetune}).
\end{enumerate}
...
To validate that our two design dimensions are orthogonal and composable, we conduct a full $2 \times 2$ ablation at two scales: a 10M nanoGPT model (Appendix~\ref{app:nanogpt}) for rapid iteration, and the 0.6B Qwen3 architecture for validation at scale.

\begin{table}[h]
\centering
\caption{Full $2 \times 2$ ablation at 0.6B scale: source representation $\times$ V-stream routing. Both dimensions provide independent gains; their combination achieves the best result. Per-stream V-routing adds only 57K parameters (0.011\%).}
\label{tab:ablation}
\begin{tabular}{llcccc}
\toprule
\textbf{Source} & \textbf{V-Routing} & \textbf{WT2 PPL $\downarrow$} & \textbf{LAM $\uparrow$} & \textbf{Hella $\uparrow$} & \textbf{$\Delta$ WT2 vs Base} \\
\midrule
\multicolumn{2}{l}{Baseline (no routing)} & 60.21 & 0.082 & 0.325 & --- \\
\midrule
Cumulative ($h_i$) & Shared & 68.87 & 0.064 & 0.310 & +14.4\% \\
Cumulative ($h_i$) & Per-stream V & \multicolumn{4}{c}{\textit{TODO: run full\_v at 0.6B}} \\
\midrule
Delta ($v_i$) & Shared & 55.69 & 0.114 & 0.340 & $-$7.5\% \\
\textbf{Delta ($v_i$)} & \textbf{Per-stream V} & \multicolumn{4}{c}{\textit{TODO: run delta\_v at 0.6B}} \\
\bottomrule
\end{tabular}
\end{table}

% TODO: fill in 0.6B full_v and delta_v results, then update this paragraph
At the 10M nanoGPT scale (Appendix~\ref{app:nanogpt}), the results reveal a clean factorization: delta sources provide $-$0.109 val loss improvement (the larger effect), while per-stream V-routing provides $-$0.057 (complementary). Their combination achieves $-$0.189, close to the sum of individual effects, confirming near-additive composability. We expect the same pattern at 0.6B scale once the remaining runs complete.

At 100M Qwen3 scale, delta sources reduce WT2 PPL from 77.69 (baseline) to 72.29 ($-$6.9\%), and adding per-stream V-routing further reduces it to 71.11 ($-$8.5\%), consistent with the nanoGPT ablation.

\subsection{Scaling to 1.7B}

\begin{table}[h]
\centering
\caption{From-scratch results at 1.7B scale ($d{=}2048$, $L{=}28$). At 20K steps the model is undertrained ($<$1 token/parameter); extending to 50K steps yields 20.9\% PPL reduction for the baseline alone.}
...
Delta AttnRes & 50K & \multicolumn{4}{c}{\textit{TODO: complete 50K run}} \\
\bottomrule
\end{tabular}
\end{table}

At 1.7B with 20K steps (1.3B tokens, $<$1 token/parameter), Delta AttnRes provides a small improvement (WT2 PPL 41.42 vs.\ 42.40, $-$2.3\%). The model is severely undertrained at this scale---the baseline itself improves by 20.9\% when extended to 50K steps. At 0.6B, routing advantages emerged clearly at 20K steps because the model receives ${\sim}2.6$ tokens/parameter; at 1.7B, achieving similar token-per-parameter ratios requires ${\sim}90$K steps. We leave extended 1.7B experiments to future work, noting that the Kimi team~\citep{kimi2025attention} observed AttnRes gains at this scale only with ${\geq}$100K steps training.
...
\paragraph{Cross-Layer Connections.} DenseNet~\citep{huang2017densely} concatenates all previous layer outputs, incurring $O(L^2 d)$ memory. DenseFormer~\citep{pagliardini2024denseformer} introduces static learned Depth-Weighted Average (DWA) weights---more efficient but not input-dependent. Hyper-Connections~\citep{zhu2024hyperconnections} widen the residual stream to $n$ parallel channels with learned mixing matrices; mHC~\citep{deepseek2025mhc} constrains these to the doubly stochastic manifold for stability at scale. MUDDFormer~\citep{wang2024muddformer} generates \emph{dynamic}, position-dependent cross-layer weights via a small MLP and crucially decouples them across Q/K/V/residual streams, demonstrating that 2.8B MUDDPythia matches 6.9B Pythia. Attention Residuals~\citep{kimi2025attention} use softmax attention over previous outputs, demonstrating improved scaling laws at up to 16B parameters. Our work identifies two overlooked dimensions in this design space: the source representation (delta vs.\ cumulative) and per-stream routing within the attention-based framework, achieving MUDDFormer-style stream decoupling with $10\times$ fewer parameters and interpretable softmax weights.

\paragraph{Residual Stream Analysis.} \citet{veit2016residual} show that residual networks behave like ensembles of shallow networks. \citet{elhage2021mathematical} formalize the ``residual stream'' view where each layer reads from and writes to a shared state. Recent work on residual stream duality~\citep{zhang2026duality} shows depth-axis attention is mathematically dual to short sliding-window attention over the sequence axis. Our source redundancy analysis (\S\ref{sec:redundancy}) complements these perspectives: we show that the \emph{representation} of cross-layer sources determines whether the model can learn selective routing---cumulative states collapse this capacity while delta sources preserve it.

\paragraph{Gating and Routing.} GLU variants~\citep{shazeer2020glu} apply gating within feed-forward layers. ReZero~\citep{bachlechner2020rezero} initializes residual branches with zero scalars. Mixture-of-Experts~\citep{shazeer2017moe,fedus2022switch} use sparse gating to route tokens across experts (the width dimension); our work routes information across \emph{depth} instead. Deep Delta Learning~\citep{zhang2026ddl} parameterizes depth updates as Householder reflections, providing geometric control; our approach uses attention-based selection over an explicit source history instead.

%% ============================================================
\section{Conclusion}
%% ============================================================

We demonstrate that two overlooked design dimensions---\emph{what to route} and \emph{where to route}---substantially impact the effectiveness of cross-layer information routing in transformers:
\begin{enumerate}
    \item \textbf{Delta sources} resolve the source redundancy problem (cumulative states have 0.95 cosine similarity, collapsing softmax to near-uniform), providing 7.5--19.2\% PPL reduction.
    \item \textbf{Per-stream V-routing} provides orthogonal, complementary gains on top of both source types. Combining delta sources with V-routing achieves the best results across all configurations.
    \item \textbf{Additive formulation} enables effective fine-tuning of pretrained models with zero initialization disruption.
\end{enumerate}
All improvements require only 0.034\% additional parameters and $<$1\% compute overhead. Code and pretrained weights are available at \url{https://github.com/wdlctc/open-attention-residuals}.
...
To enable rapid iteration over the full $2 \times 2$ design space, we implement all variants in nanoGPT~\citep{radford2019gpt2} and train on Shakespeare character-level data ($L{=}6$, $d{=}384$, ${\sim}$10M parameters, 5K iterations on a single GPU). While this scale is too small for definitive conclusions, it provides a controlled testbed where all five configurations run in minutes with identical hyperparameters.

\begin{table}[h]
\centering
\caption{nanoGPT Shakespeare ablation: $2 \times 2$ source $\times$ stream design space. Both dimensions provide independent gains that compose nearly additively.}
\label{tab:nanogpt}
\begin{tabular}{llcc}
\toprule
...
Key findings:
\begin{itemize}
    \item \textbf{Delta source effect}: $-$0.109 (6.189 $\to$ 6.080), the larger of the two effects.
    \item \textbf{Per-stream V effect}: $-$0.057 (6.189 $\to$ 6.132), consistent across both source types.
    \item \textbf{Composability}: $-$0.189 total $\approx$ $-$0.153 $+$ $-$0.036, confirming near-additive composition.
\end{itemize}
