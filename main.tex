...
\begin{abstract}
Attention Residuals~\citep{kimi2025attention} replace standard additive residual connections with learned softmax attention over previous layer outputs, enabling selective cross-layer information routing. However, two critical design choices have been overlooked: (1)~the \emph{source representation}---what to attend over, and (2)~whether different \emph{information streams} (Q, K, V) within each layer should share the same routing. We address both. First, we show that \textbf{Delta Attention Residuals}---attending over per-sublayer outputs $v_i$ rather than cumulative states $h_i$---reduces WikiText-2 perplexity by 19.2\% relative to cumulative-state AttnRes and 7.5\% relative to the baseline, because cumulative states exhibit 0.95 cosine similarity that collapses softmax routing. Second, we introduce \textbf{per-stream delta routing}, which decouples the depth-axis routing for the value stream from the query/key streams. Since values carry content information while queries/keys encode positional matching, they benefit from attending to different sublayer outputs across depth. Per-stream routing adds only 0.011\% parameters while further improving over standard Delta AttnRes. Delta AttnRes's additive formulation also enables effective fine-tuning of pretrained models, achieving 29.2\% PPL reduction on Qwen3-0.6B. We release code, pretrained weights, and visualization tools at \url{https://github.com/wdlctc/open-attention-residuals}.
\end{abstract}
...
Yet a critical design choice in Attention Residuals has been overlooked: the \emph{source representation}---what the model attends over. The paper defines sources as individual sublayer outputs $v_i = f_i(h_i)$ (the ``deltas'' added at each layer), but a seemingly equivalent implementation stores cumulative hidden states $h_i = h_0 + \sum_{j=1}^{i} v_j$ instead. After all, one is a linear combination of the other. We show these choices are far from equivalent.
...
\begin{enumerate}
    \item We identify the \textbf{source redundancy problem}: cumulative-state implementations produce sources with 0.95 cosine similarity, degrading softmax routing to near-uniform (\S\ref{sec:analysis}).
    \item We demonstrate that \textbf{Delta Attention Residuals}---attending over per-sublayer outputs---consistently outperforms cumulative-state variants, achieving 7.5--19.2\% PPL reduction across scales (\S\ref{sec:experiments}).
    \item We introduce \textbf{per-stream delta routing}, which decouples value-stream depth routing from query/key routing. This enables the value stream to retrieve content from different depths than the positional matching performed by Q/K, adding only 0.011\% parameters (\S\ref{sec:per_stream}).
...
\subsection{Per-Stream Delta Routing}
\label{sec:per_stream}

In standard Delta AttnRes, both Q/K and V projections within the self-attention sublayer receive the \emph{same} depth-routed input $\hat{h}_l$. However, Q/K and V serve fundamentally different roles: Q/K compute positional/relational matching (``where to attend''), while V carries the content that gets propagated (``what to retrieve''). There is no reason these streams should prefer the same cross-layer sources.

A natural idea is to give each stream its own routing query, as explored by MUDDFormer~\citep{wang2024muddformer} for MLP-generated weights. However, there is a subtle but critical prerequisite: \textbf{per-stream routing only works when the sources are sufficiently distinctive for the softmax to discriminate.}

Consider applying per-stream routing to cumulative-state AttnRes. With adjacent source cosine similarity $\sim$0.95, the softmax logits $w^{\top} \mathrm{RMSNorm}(h_i)$ are nearly identical across sources regardless of the query $w$. Two independent queries $w^{qk}$ and $w^{v}$ would both produce near-uniform distributions---per-stream routing degenerates to shared routing. In contrast, delta sources with cosine similarity $<$0.3 provide a basis rich enough for different queries to learn genuinely different routing patterns: $w^{v}$ can concentrate on MLP deltas (semantically rich) while $w^{qk}$ focuses on attention deltas (positional information), because the softmax can actually distinguish between these structurally different sources.

This reveals an important connection: \textbf{delta sources do not merely fix the routing collapse problem (\S\ref{sec:redundancy})---they unlock per-stream specialization as an emergent capability that is fundamentally unavailable to cumulative-state variants.}

We propose \textbf{per-stream delta routing} (Delta-V), which gives the value projection its own independent depth-routing query:
\begin{align}
...
\paragraph{Contrast with MUDDFormer.} MUDDFormer~\citep{wang2024muddformer} also decouples per-stream routing, but uses unconstrained MLP-generated weights (no softmax normalization). Because their weights are not softmax-normalized, source diversity is not a prerequisite---any linear combination can be learned regardless of source similarity. Our approach operates within the softmax attention framework, which brings interpretability (routing weights sum to 1, can be visualized as attention maps) but imposes the constraint that sources must be distinctive for per-stream queries to specialize. Delta sources satisfy this constraint; cumulative sources do not.
...
\subsection{Per-Stream Routing Results}
\label{sec:per_stream_results}

\begin{table}[h]
\centering
\caption{Effect of per-stream delta routing (V-stream decoupling). Delta-V adds an independent routing query for the value stream in each attention sublayer. All runs use identical hyperparameters and are evaluated on the same machine for fair comparison.}
\label{tab:delta_v}
...
At 100M scale, Delta-V AttnRes achieves the lowest WikiText-2 perplexity (71.11), reducing PPL by 8.5\% relative to the baseline and 1.6\% relative to standard Delta AttnRes. Crucially, the improvement from Delta to Delta-V validates the analysis in \S\ref{sec:per_stream}: delta sources are distinctive enough (cos sim $<$0.3) for two independent queries to learn different routing patterns, allowing the V-stream to specialize in content retrieval while Q/K handles positional matching. This specialization would be impossible with cumulative sources, where softmax collapse forces all queries toward the same near-uniform distribution regardless of their initialization.

At this small scale (100M, 20K steps), LAMBADA and HellaSwag differences are within noise---the model has insufficient capacity for these tasks to differentiate routing strategies. WikiText-2 perplexity, which aggregates over a larger evaluation corpus, provides the clearest signal. The 0.6B results (in progress) are expected to show larger separations across all metrics, consistent with the pattern observed for Delta vs.\ baseline (Table~\ref{tab:scratch_0.6b}).
...
Our findings:
\begin{enumerate}
    \item \textbf{Source representation determines routing quality}: Cumulative states have 0.95 cosine similarity, collapsing softmax to near-uniform. Delta sources are naturally distinctive.
    \item \textbf{Delta AttnRes outperforms all cumulative-state variants}: 19.2\% PPL reduction vs.\ Full AttnRes and 7.5\% vs.\ baseline at 0.6B scale, with +39\% LAMBADA accuracy.
    \item \textbf{Delta sources unlock per-stream specialization}: Because delta sources are structurally distinctive, independent routing queries for Q/K and V can learn genuinely different depth patterns. This per-stream capability is unavailable to cumulative-state variants where softmax collapse forces all queries toward the same distribution.
